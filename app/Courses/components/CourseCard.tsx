...
export default function CourseCard({ course }: CourseCardProps) {
    ...
    if (event.data === window.YT.PlayerState.PLAYING) {
      const interval = setInterval(() => {
        if (player && player.getCurrentTime && player.getDuration) {
          const newProgress = (player.getCurrentTime() / player.getDuration()) * 100;
  ...
  const extractLearnSection = (content: string) => {
    const learnMatch = content.match(/#### What (?:you'll|you will) learn([\s\S]*?)(?=####|$)/i);
    return learnMatch ? learnMatch[1].trim() : content.split('\n')[0];
  };

  const contentPreview = extractLearnSection(course.content);
  const showSeeMore = contentPreview.length > 100 || course.content.includes('####');
