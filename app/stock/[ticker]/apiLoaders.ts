import { companyNewsResult, companyProfileResult, earningsResult, marketStatusResult, quoteResult } from "@/app/type_definitions"
...
export async function analyst_ratings(symbol: string){
    const recommendation_request = await fetch(`${base_URL}/stock/recommendation?symbol=${symbol}&token=${api_key}`)
